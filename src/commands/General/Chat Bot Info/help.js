...
module.exports = class extends Command {
	...
	async run(message, [command]) {
		...
		return message[method].send(helpMessage, { split: { char: '\u200b' } })
			...
			// I changed the catch block to send help to channel if can't dm
			.catch(() => { if (message.channel.type !== 'dm' && this.client.user.bot) message.channel.send(helpMessage, { split: { char: '\u200b' } }); });
	}
